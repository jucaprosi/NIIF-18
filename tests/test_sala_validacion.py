...
def test_contar_cuentas_pendientes():
    df = pd.DataFrame({"Categoria_NIIF18": ["1. Operación (Ingresos / Gastos Operativos)", "CATEGORIA_INVALIDA", None]})
    assert val_svc.contar_cuentas_pendientes(df, ["1. Operación (Ingresos / Gastos Operativos)"]) == 2


def test_puede_exportar_bloquea_si_no_cuadra():
    df = pd.DataFrame({
        "Saldo": [100.0, -50.0],
        "Categoria_NIIF18": ["1. Operación (Ingresos / Gastos Operativos)"] * 2,
    })
    ok, motivos = val_svc.puede_exportar(df, ["1. Operación (Ingresos / Gastos Operativos)"])
    assert ok is False
...
def test_puede_exportar_bloquea_si_hay_pendientes():
    df = pd.DataFrame({
        "Saldo": [100.0, -100.0],
        "Categoria_NIIF18": ["1. Operación (Ingresos / Gastos Operativos)", "CATEGORIA_INVALIDA"],
    })
    ok, motivos = val_svc.puede_exportar(df, ["1. Operación (Ingresos / Gastos Operativos)"])
    assert ok is False
